...
class ScanProcessor:
    ...
    def _scan_secrets(scan_request, repo_path, trufflehog_version):
        ...
        """
        try:
            logger.info(f"Запуск TruffleHog {trufflehog_version} для сканирования секретов: {repo_path}")

            # Для brew-версии используем только v3 команды
    ...
    def _scan_with_trufflehog_brew(repo_path):
        ...
        """
        command_variants = [
            # Основная команда для brew версии
            ...
            ['trufflehog', 'filesystem', repo_path, '--json', '--since-commit', 'HEAD~100'],
        ]
        ...
        for i, scan_args in enumerate(command_variants):
            ...
            try:
                ...
                logger.info(f"TruffleHog brew.{i+1} завершился с кодом: {result.returncode}")
                logger.info(f"Stdout длина: {len(result.stdout)}")
                logger.info(f"Stderr: {result.stderr[:500] if result.stderr else 'None'}")
    ...
    def _parse_trufflehog_output(output):
        """
        Парсит вывод TruffleHog в формате JSON
        """
        findings = []
        valid_lines = 0
        invalid_lines = 0
            ...
            if line.strip():
                try:
                    ...
                    if isinstance(finding, dict):
                        ...
                        detector_name = (finding.get('DetectorName') or
                    ...
                    logger.warning(f"Не удалось распарсить JSON: {line[:100]}..., ошибка: {e}")
                    invalid_lines += 1
                    continue

        logger.info(f"Парсинг завершен: {valid_lines} валидных, {invalid_lines} невалидных строк")
        return findings
    ...
    def _save_secret_finding(scan_request, finding):
        ...
        """
        try:
            # Извлекаем информацию о finding (поддержка разных форматов TruffleHog v3)
            ...
            line_number = (finding.get('Line') or
                           finding.get('line') or
                           finding.get('LineNumber') or
                           finding.get('lineNumber') or
                           1)

            secret_type = (finding.get('DetectorName') or
                           finding.get('detectorName') or
                           finding.get('Detector') or
                           finding.get('detector') or
                           'Unknown Secret')
    ...
    def _scan_dependencies(scan_request, repo_path):
        """
        Сканирует зависимости (базовая реализация)
        """
        try:
            ...
            for root, dirs, files in os.walk(repo_path):
                ...
                for file in files:
                    if file in ['requirements.txt', 'package.json', 'pom.xml', 'build.gradle', 'composer.json']:
    ...
    def start_scan_async(scan_request_id):
        ...
        """
        thread = threading.Thread(target=ScanProcessor.process_scan, args=(scan_request_id,))
        thread.daemon = True
        thread.start()